...
import { EntityArrayResponseType, EntitySubscriptionService } from '../service/entity-subscription.service';
...
export class EntitySubscriptionComponent implements OnInit {
  ...
  protected readonly entitySubscriptionService = inject(EntitySubscriptionService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IEntitySubscription): number => this.entitySubscriptionService.getEntitySubscriptionIdentifier(item);
  ...
  delete(entitySubscription: IEntitySubscription): void {
    const modalRef = this.modalService.open(EntitySubscriptionDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.entitySubscription = entitySubscription;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && EntitySubscriptionComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.entitySubscriptionService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
    return this.entitySubscriptionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
